...
class SimulationNN(nn.Module):
    def __init__(self, num_states, num_actions, learningStd=False):
        ...
        self.num_states = num_states #-1 # minus the task indicator
        ...
        self.p_film1=FiLM(self.num_h1,1)
        self.p_film2=FiLM(self.num_h2,1)
        self.p_film3=FiLM(self.num_h3,1)
        self.p_film4=FiLM(self.num_actions,1)

        self.v_film1=FiLM(self.num_h1,1)
        self.v_film2=FiLM(self.num_h2,1)
        self.v_film3=FiLM(self.num_h3,1)
        self.v_film4=FiLM(1,1)

        # self.p_ln1=nn.LayerNorm(self.num_h1)
        # self.p_ln2=nn.LayerNorm(self.num_h2)
        # self.p_ln3=nn.LayerNorm(self.num_h3)

        # self.v_ln1=nn.LayerNorm(self.num_h1)
        # self.v_ln2=nn.LayerNorm(self.num_h2)
        # self.v_ln3=nn.LayerNorm(self.num_h3)
    ...
    def reset(self):
        ...
        weights_init(self.p_fc1) #.apply(weights_init)
        weights_init(self.p_fc2) #.apply(weights_init)
        weights_init(self.p_fc3) #.apply(weights_init)
        weights_init(self.p_fc4)

        weights_init(self.v_fc1) #.apply(weights_init)
        weights_init(self.v_fc2) #.apply(weights_init)
        weights_init(self.v_fc3) #.apply(weights_init)
        weights_init(self.v_fc4)

        self.p_film1.apply(weights_init)
        self.p_film2.apply(weights_init)
        self.p_film3.apply(weights_init)
        self.p_film4.apply(weights_init)

        self.v_film1.apply(weights_init)
        self.v_film2.apply(weights_init)
        self.v_film3.apply(weights_init)
        self.v_film4.apply(weights_init)

    def forward(self, x):
        #print("obs shape= ",x.shape)
        if len(x.shape)>1:
            task_indicator = x[:,0].unsqueeze(-1)
            state = x[:,1:]
        else:
            task_indicator = x[0].unsqueeze(-1)
            state = x[1:]           
        #print("task=",x[:,0]," mStride=",x[:,-279]," mCadence=",x[:,-278])
        y = self.p_fc1(x)
        #y =  self.p_ln1(y)
        y = self.p_film1(y,task_indicator)
        y = F.relu(y)

        y = self.p_fc2(y)
        #y = self.p_ln2(y)
        y = self.p_film2(y,task_indicator)
        y = F.relu(y)

        y = self.p_fc3(y)
        #y = self.p_ln3(y)
        y = self.p_film3(y,task_indicator)
        y = F.relu(y)

        y = self.p_fc4(y)
        #y = self.p_ln4(y)
        y = self.p_film4(y,task_indicator)

        p_out = MultiVariateNormal(y, self.log_std.exp())

        y = self.v_fc1(x)
        #y =  self.v_ln1(y)
        y = self.v_film1(y,task_indicator)
        y = F.relu(y)

        y = self.v_fc2(y)
        #y =  self.v_ln2(y)
        y = self.v_film2(y,task_indicator)
        y = F.relu(y)

        y = self.v_fc3(y)
        #y =  self.v_ln3(y)
        y = self.v_film3(y,task_indicator)
        y = F.relu(y)

        #v_out = self.v_fc4(y)
        y = self.v_fc4(y)
        v_out = self.v_film4(y,task_indicator)
